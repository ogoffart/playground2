...
For non-trivial apps, use **two globals**: one carrying data *into* the UI, one
carrying callbacks *out*. This avoids threading properties/callbacks through every
component.
...
camelCase in JS). The two-globals pattern and the "logic in host, view-model into
`.slint`" split apply identically.
